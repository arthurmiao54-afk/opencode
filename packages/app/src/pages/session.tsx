...
export default function Page() {
  ...
  const sessionPanelWidth = createMemo(() => {
    const fileTreeWidth = desktopFileTreeOpen() ? layout.fileTree.width() : 0
    const reviewWidth = desktopReviewOpen() ? layout.session.width() : 0
    const totalSideWidth = fileTreeWidth + reviewWidth
    if (totalSideWidth === 0) return "100%"
    return `calc(100% - ${totalSideWidth}px)`
  })
